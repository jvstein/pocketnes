...
void serialinterrupt() {
	u32 read=REG_SIOMULTI0;
	u32 exram;
	int c;
	switch(sendmode) {
		case 0:				//first data should be 0x002E (first word in header)
			if(read!=0x2e) {		//assume anything else is the transfer size
				REG_SIOMLT_SEND=read;	//reply with same size
				read<<=2;
				totalbytes=read;
				break;
			} else
...
void AGBmain() {
	...
	while(sendmode<2) {	//while transfer not complete
		scrollpos--;
		REG_BG1VOFS=scrollpos;
		REG_BG0VOFS=96+160*count/totalbytes;

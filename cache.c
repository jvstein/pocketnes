...
void assign_prg_pages(u8* base, int start, int end)  //in units of 8k
{
	int i;
	for (i=start;i<end;i++)
	{
		instant_prg_banks[i]=base+i*8192;
	}
}
void assign_prg_pages2(u8* base, int start, int numpages)
{
	assign_prg_pages(base-start*8192,start,start+numpages);
}
...
void make_contiguous(u8* cachebase, int page1, int page2) //16k page size
{
	...
	u8* bank3_addr=instant_prg_banks[page1*2];
	u8* bank4_addr=instant_prg_banks[page3*2];
	u8* bank7_addr=instant_prg_banks[page2*2];
	...
	{
		swap_prg_pages(page3*2,page2*2,1*2);
	}
	//Rom inside ROM
	if (bank3_memtype==8)
	{
		...
		assign_prg_pages2(bank3_dest,page1*2,1*2);
	}
...
void get_rom_map()
{
	...
	const u32 sram_w_1 = (u32)sram_W2;
	...
	if (using_flashcart() && (cartflags & SRAM) )
	{
